...
function validate(name, target, swaggerModel, swaggerModels, allowBlankTargets, disallowExtraProperties, customValidators) {
    ...
    if(subModelName) {
        var swaggerSubModel = swaggerModels[subModelName];

        if(swaggerSubModel) {
            if (swaggerSubModel !== swaggerModel) {
                return validate(name, target, swaggerSubModel, swaggerModels, allowBlankTargets, disallowExtraProperties, customValidators);
            }
        } else {
            return createReturnObject(new Error('Unable to validate against an undefined sub-model.'));
        }
    }
