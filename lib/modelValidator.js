...
function Validator(swagger) {
    ...
    if(swagger) {
        swagger.validateModel = function(modelName, obj, allowBlankTargets, disallowExtraProperties) {
            ...
            return validate(modelName, obj, model, models, allowBlankTargets, disallowExtraProperties, self.customValidators);
        };
    }
}
...
Validator.prototype.validate = function(target, swaggerModel, swaggerModels, allowBlankTarget, disallowExtraProperties) {
    ...
    var swaggerModelName = 'rootModel';
    if(isStringType(swaggerModel) && swaggerModels[swaggerModel]) {
        swaggerModelName = swaggerModel;
        swaggerModel = swaggerModels[swaggerModel];
    }

    return validate(swaggerModelName, target, swaggerModel, swaggerModels, allowBlankTarget, disallowExtraProperties, this.customValidators);
};
...
function validate(name, target, swaggerModel, swaggerModels, allowBlankTargets, disallowExtraProperties, customValidators) {
    ...
    var subModelName = swaggerModel && swaggerModel.discriminator && target[swaggerModel.discriminator];
    if(subModelName) {
        swaggerModel = swaggerModels[subModelName];
    }
...
function validateType(name, property, field, models, allowBlankTargets, disallowExtraProperties) {
    ...
    // Only attempt to validate an object if properties or additionalProperties are defined in the model.
    ...
    if(expectedType === 'object') {
        if(field.properties) {
            ...
            if (field.additionalProperties.type) {
                let fieldType = field.additionalProperties.type;
                let arrayErrors1 = [];
                for (let key in property) {
                    let errors1 = validateValue(key, field.additionalProperties, property[key], models, allowBlankTargets, disallowExtraProperties);
                    if (errors1) {
                        arrayErrors1.push(...errors1);
                    }
                }
                if(arrayErrors1.length > 0) {
                    return createReturnObject(arrayErrors1, "Additional properties of type " + fieldType + " (" + name + ")");
                }
            } else if (models && field.additionalProperties.$ref) {
                let fieldRef2 = field.additionalProperties.$ref.replace('#/definitions/', '');
                let model = models[fieldRef2];
                if (model) {
                    let arrayErrors2 = [];
                    for (let key in property) {
                        let errors2 = validate(key, property[key], model, models, allowBlankTargets, disallowExtraProperties);
                        if(errors2 && !errors2.valid) {
                            arrayErrors2.push(...errors2.errors);
                        }
                    }
                    if(arrayErrors2.length > 0) {
                        return createReturnObject(arrayErrors2, "Additional properties of type " + field.$ref + " (" + name + ")");
                    }
                }
            }
            return null;
    ...
    if(expectedType === 'array') {
        ...
        if(field.items && field.items.type) {
            ...
            property.forEach(function (value) {
                let errors1 = validateValue(name + count.toString(), field.items, value, models, allowBlankTargets, disallowExtraProperties);
                if(errors1) {
            ...
            if(model) {
                let arrayErrors2 = [];
                property.forEach(function(value) {
                    let errors2 = validate(name, value, model, models, allowBlankTargets, disallowExtraProperties);
                    if(errors2 && !errors2.valid) {
...
function validateRequiredFields(object, fields, modelFields) {
    ...
    for (var i = 0; i < fields.length; ++i) {
        ...
        try {
            if(!object.hasOwnProperty(property) || (object[property] === null && !modelFields[property]['x-nullable'])) {
...
function clone(source) {
    if (source === null || typeof(source) !== 'object' || '__clone' in source)
        return source;
    ...
    if(model.allOf) {
        ...
        model.allOf.forEach(function(item) {
           if(item.$ref) {
               ...
               merge(merged, clone(modelRef));

           } else {
               merge(merged, clone(item))
           }
        });
